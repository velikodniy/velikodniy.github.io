...
Если вы когда-нибудь интересовались сжатием изображений, то, возможно,
слышали о вейвлетах. Они используются, например, в формате сжатия
JPEG2000, который превосходит по качеству своего предка — JPEG.
Или в популярном формате для хранения сканированных документов DjVu.

Литература о теории вейвлетов пестрит интегралами и терминами из
теории обработки сигналов. Но на самом деле, понять, чем же хороши
вейвлеты можно и не углубляясь в сложную местами (хоть и интересную)
теорию.

Попробуем на простых примерах разобраться, как вообще сжимаются
изображения и как для этого применить вейвлета. Предполагается, что
...
Полусуммы усредняют значения яркостей, то есть «убирают»
...
и синей компонент), а Y'C<sub>B</sub>C<sub>R</sub> или подобные.
В них первая компонента
...
1 и 2). Можно зеркально отразить значения или просто подставить нули.
Правильный выбор соглашения
...
Конечно, в этой статье не были рассмотрены очень многие интересные математические детали и практические применения вейвлет-преобразований.
На практике применяются более сложные, но и более эффективные подходы.
Разумеется, это только самые основы.
И на самом деле вейвлеты конструируются не так, как описано.
Но многое сложно объяснить не повышая градус матана.
Надеюсь, что у читателя после прочтения этой статьи появилось общее представление о том, как используются вейвлеты в области сжатия изображений.

Литература
==========

См. также мою статью [«Схема лифтинга»]({{ site.url }}/2020/05/25/lifting-scheme/).
...
- [Протасов В. Всплески Ингрид Добеши](https://elementy.ru/nauchno-populyarnaya_biblioteka/434626/Vspleski_Ingrid_Dobeshi).