...
низкой частотой. Решающую эту задачу части схем называют
...
питания потечёт через резистор R1, базу и эммитер на землю. При этом
транзистор откроется (если, конечно, ток достаточно большой), и ток
сможет идти через переход коллектор — эммитер, а значит и через
нагрузку.

Резистор R1 играет важную роль — он ограничивает ток через переход
база — эммитер. Если бы его не было, ток не был бы ничем ограничен и
просто испортил бы транзистор. А заодно и управляющую микросхему (ведь
именно она связывает линию питания с транзистором). Максимальный ток
через один выход микроконтроллера обычно ограничен значением около 25
мА (для STM32).

В интернете можно встретить утверждения, что микроконтроллеры AVR
выдерживают ток в 200 мА, но это относится ко всем выводам в
сумме. Предельное допустимое значение тока на один вывод примерно
такое же — 20-40 мА.
...
есть «сверху». Если подключить её «снизу», то управляющий ток будет
идти и через неё тоже, а так как сопротивление нагрузки может быть
большим, транзистор может просто не открыться.
...
коллектора и напряжении коллектор — эммитер.
...
Модели выбраны случайно, просто это модели, которые легко найти или
откуда-то выпаять. Для ключа в рассматриваемой схеме, конечно, можно
использовать любой n-p-n-транзистор, подходящий по параметрам и цене.
...
Для нагрузки в виде реле можно добавить ешё несколько
...
Для тока $I_{LED} = 0{,}075\,А$ управляющий ток должен быть в $\beta = 400$ раз меньше:
...
дополнительное падение напряжения, и транзистор может не открыться.
...
транзисторов даже из одной артии этот параметр может сильно
...
использовать в качестве ключа — это [биполярный транзисторы с
...
включения и выключения нагрузки он подходит не очень хорошо, так как
половину времени на каждый период переменного тока прибор простаивает.
Тем не менее, тиристор можно использовать в диммере. Там он может
применяться для управления мощностью, отсекая от волны питания кусочек
требуемой мощности.

Симистор — это, фактически двунаправленный тиристор. А значит он
позволяет пропустить не полуволны, а полную волну напряжения питания
нагрузки.
...
полярность или снизив ток через него то величины, меньшей чем ток
удержания. Но так как питание организовано переменным током, это
автоматически произойдёт по кончании полупериода.

При выборе симистора важно учесть величину тока удержания
($I_H$). Если взять мощный симистор с большим током удержания, ток
через нагрузку может оказаться слишком маленьким, и он просто не
откроется.
...
использовать дополнительный ключ.

Встроенный симистор же рассчитан на напряжение до 600 В и ток до 1
А. Этого достаточно для управления мощными бытовыми приборами через
...
Таким образом, эта оптопара выстуает в роли драйвера
симистора.
...
действующего напряжения. Пиковое напряжение равно $\sqrt2 220 \approx
310 В$.
...
большее пикового в сети питания. Напомним, что для 220 В — это 310
В. Лучше брать с запасом.
...
Реле покупают своей простотой и эффективностью. Например, реле
...
Части этих недостатков лишены так называемые [твердотельные
...
Таким образом, в арсенале у нас достаточно сособов управления
